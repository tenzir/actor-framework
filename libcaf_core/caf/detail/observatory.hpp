...
public:
    ...
    // into `target`. Will not allocate.
    // Returns number of bytes written. If the result is equal to `maxlen`,
    // the caller *must* siphon again since the beginning of the stream may
    // contain an incomplete event in this case.
    // Can be used while still recording, if desired.
    size_t siphon(char* target, size_t maxlen);
...
inline size_t BoundedRecorder::siphon(char* target, size_t maxlen)
{
    auto head = *head_;
    auto tail =  *tail_;
    auto size = head - tail;
    if (size > maxlen)
        size = maxlen;
    auto buffer_start = tail % ring_size_;
    auto buffer_end = head % ring_size_;
    // We can't use the double ringbuffer trick since perf injects 1 page
    // of control data at the start of the mmap area.
    if (buffer_end < buffer_start) {
        size_t size_eob = std::min(ring_size_ - buffer_start, size);
        memcpy(target, ring_base_ + buffer_start, size_eob);
        // Copy remainder from the beginning of the buffer,
        // if necessary.
        if (size > size_eob)
            memcpy(target + size_eob, ring_base_, size - size_eob);
    } else {
        memcpy(target, ring_base_ + buffer_start, size);
    }
    // Update tail to new position
    *tail_ = tail + size;
    // DEBUG: Verify.
    // auto *cursor = target;
    // while (cursor < target + maxlen) {
    //     auto hdr = (struct perf_event_header*)target;
    //     fprintf(stderr, "event %d, size %d\n", hdr->type, hdr->size);
    //     cursor += hdr->size;
    // }
    return size;
}

/// This class contains a one `BoundedRecorder` per thread.
/// Every call to `enable()`/`disable()` only applies to the
/// recorder for the current thread.
class ThreadAwareRecorder {
public:
    ThreadAwareRecorder(
...
inline const std::string& ThreadAwareRecorder::filename() const {
    return filename_;
}
    ...
    struct perf_event_header* suffix)
{
    ...
    MemoryArea area;
    auto* array = new char[size+1];
    area.data = std::unique_ptr<char[]>(array);
    area.reserved = size;
    area.used = 0;
    ...
    // FIXME: while (recorder.siphon(...) == max_size) {...}
    area.used += recorder.siphon(area.data.get() + area.used, area.reserved - area.used);
